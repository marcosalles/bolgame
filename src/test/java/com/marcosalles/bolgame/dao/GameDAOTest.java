...
public class GameDAOTest {
	...
	public void save__should_not_save_without_id() {
		final var game = Game.builder()
			.playerOne(this.player("player-1"))
			.playerTwo(this.player("player-2"))
			.build();
		gameDAO.save(game);
	}
	...
	public void findByParticipantId__should_return_null_if_no_games_with_participant() {
		final var playerOne = this.player("player-1");
		final var playerTwo = this.player("player-2");
		final var playerThree = this.player("player-3");
		gameDAO.save(Game
			.builder()
			.id("game-1")
			.playerOne(playerOne)
